...
def write_group_membership_report(course):
    ...
    with (course.dir / "groups.csv").open("w") as file:
        ...
        for canvas_user in course.canvas_course.students:
            ...
            } | {lab.name: f(lab) for lab in course.labs.values()}
...
def read_report_skeleton(course, path):
    ...
    with (course.dir / path).open() as file:
        ...
        reader = csv.DictReader(
            file, fieldnames=[header_personnummer, header_name], dialect=LadokDialect
        )
        # Ignore header row.
        it = iter(reader)
        next(it)
    ...
    * examination_date:
    ...
    def f():
        for entry in requested_entries:
            ...
            if gitlab_username is None:
                print(
                    f"WARNING: No Chalmers GitLab user found Canvas user {canvas_user.name}"
                )
                continue
