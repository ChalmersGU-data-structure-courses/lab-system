...
def make_students_developers():
    ...
    for n, group in lab_groups.items():
        for user in group.members.list():
            if user.access_level != gitlab.const.AccessLevel.OWNER:
                if user.access_level != gitlab.const.AccessLevel.DEVELOPER:
                    print(
                        f"changing rights of {user.name} in {group.name} to developer..."
                    )
                    user.access_level = gitlab.const.AccessLevel.DEVELOPER
                    user.save()
...
def check_submissions(lab):
    ...
    for n, group in lab_groups.items():
        project = gl.projects.get(
            from_singleton(p.id for p in group.projects.list() if p.path == f"lab{lab}")
        )
...
def create_submission_branch(n, tag):
    subprocess.run(
        ["git", "fetch", "--tags", "--force", "--prune", "g{}".format(n)],
        cwd=repo,
        check=True,
    )
