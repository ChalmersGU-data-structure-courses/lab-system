...
class GUIDtoCID(path_tools.AttributeJSONCache):
    ...
    def ensure_entries(self, canvas_users):
        def process(self, canvas_user):
            ...
            if updated:
                self.mapping[guid] = self.do_lookup(guid, canvas_name, canvas_name_sortable)
            return updated
    ...
    def do_lookup(self, guid, canvas_name, canvas_name_sortable = None):
        ...
        try:
            ...
            logging.warning(f'Could not resolve GU ID {guid} with Canvas name {canvas_name}.')
            if not results:
                logging.warning('No matches with that name on Chalmers LDAP.')
            else:
                logging.warning(f'Multiple matches name on Chalmers LDAP: {", ".join(cids)}')
            return None
