...
def with_use_uids(connection, cmd, use_uids):
    return (
        functools.partial(connection.uid, cmd) if use_uids else getattr(connection, cmd)
    )
...
def segment_parse_uid(segment):
    match = re.match(rb"\s*UID ([\d]+)", segment)
...
pp_flag = print_parse.regex_non_canonical(b"\\{}", rb"\\(.*)")
...
def segment_parse_flags(segment):
    print(segment)
    match = re.match(rb"\s*FLAGS (\([^)]*\))", segment)
...
def process_emails_from(connection, date, decide_if_process, process):
    ...
    * process:
    ...
    while True:
        ...
        if uid_start:
            metadata = fetch(
                connection, message_set_range(uid_start + 1), [spec_flags, spec_headers]
            )
...
with connection_context(
    "imap.chalmers.se", user="sattler", password="Gw<G#9Q8DV"
) as connection:
    connection.select("INBOX")
