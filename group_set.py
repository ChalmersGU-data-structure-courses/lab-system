...
class GroupSet:
    ...
    def canvas_group_set_get(self, use_cache):
        return canvas.GroupSet(self.canvas_course, self.config.group_set_name, use_cache = use_cache)
    ...
    def create_groups_on_canvas(self, group_ids):
        ...
        '''
        group_names = general.sdict((group_id, self.config.name.print(group_id)) for group_id in group_ids)
    ...
    def groups_on_canvas(self):
        return dict(
            (self.config.name.parse(canvas_group.name), tuple(
