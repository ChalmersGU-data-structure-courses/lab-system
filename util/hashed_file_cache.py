    ...
    * Emptying the case.
    ...
    with util.general.traverse_managers_list(locks()):
        ...
        if link_data is not None:
            ...
            inhabited_and_up_to_date = (
                cache_link_data is not None and cache_link_data.hash == link_data.hash
            )
        ...
        # Clean up old data.
        if cache_link_data is not None and not (
            link_data is not None and cache_link_data.hash == link_data.hash
        ):
    ...
    with util.path.dir_fd("cash") as dir_fd:
        ...
        print(
            ...
            write_cache(
                dir_fd,
                (lambda: b"1234"),
                link_data=LinkData(hash="7110eda4d09e062aa5e4a390b0a572ac0d2c0220"),
            ),
        )
        print(
            "write: ",
            write_cache(
                dir_fd,
                (lambda: b"1234235"),
                link_data=LinkData(update_date=datetime.datetime(2000, 1, 1)),
            ),
        )
        print("read: ", read_cache(dir_fd, read_update_date=True))
        print("write: ", write_cache(dir_fd))
