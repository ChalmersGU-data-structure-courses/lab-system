...
class GradingViaMergeRequest:
    ...
    @functools.cached_property
    def notes(self):
        return gitlab_tools.list_all(self.merge_request.notes, sort = 'asc')
    ...
    def notes_by_date(self):
        return [(gitlab_tools.parse_date(note.created_at), note) for note in self.notes]
        ...
        for note in self.notes:
            if note.author['id'] in self.course.lab_system_users:
                try:
                    ...
                    yield (request_name, (gitlab_tools.parse_date(note.created_at), note))
                except ValueError:
                    pass
        pass
    ...
    def reviewer_intervals(self):
        return list(gitlab_tools.parse_reviewer_intervals(self.notes))
    ...
    def label_events_generator(self):
        for label_event in gitlab_tools.list_all(self.merge_request.resourcelabelevents):
            try:
    ...
    def label_event_url(self, date):
        ...
        return gitlab_tools.url_merge_request_note(self.merge_request, note)
    ...
    def summary_table(self):
        def column_specs():
            yield markdown.ColumnSpec(title = markdown.link(
                'Submission tag',
                gitlab_tools.url_tag_name(self.group.project.lazy),
            ))
            ...
            for (request_name, (date, note)) in self.synced_submissions.items():
                ...
                def col_request_name():
                    return markdown.link(
                        request_name,
                        gitlab_tools.url_tree(self.group.project.lazy, request_name)
                    )

                def col_sync():
                    return markdown.link(
                        self.course.format_datetime(gitlab_tools.parse_date(note.created_at)),
                        gitlab_tools.url_merge_request_note(self.merge_request, note)
                    )
                ...
                def col_grader():
                    if has_outcome:
                        return markdown.link(grader, gitlab_tools.url_username(self.gl, grader))
