...
gitlab_lab_system_users = ['lab-system', 'myreen', 'oskeri']
...
#
# * primary         # Official problem and solution repository.
...
lab = SimpleNamespace(
    ...
    full_id = print_parse.from_dict({(): 'lab'}.items()),
...
grading_sheet = SimpleNamespace(
    ...
    spreadsheet = '16uGLWW0Che7SvpAaxaOCaoTDQpAcEd-8tDPeq8ObzqA',
...
_lab = SimpleNamespace(
    ...
    request_handlers = {'submission': lab_handlers_tda283.SubmissionHandler()},
    refresh_period = datetime.timedelta(minutes = 15),
    grading_via_merge_request = True,
    outcome_labels = {
        id: gitlab_tools.LabelSpec(name = _outcomes.get(id, 'waiting-for-grading'), color = color)
